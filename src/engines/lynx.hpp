...
const static size_t numParameters = psqtIndexCount +
                                    ...
                                    PassedPawnPushEnemyBonus.size +                // PSQTBucketCount * 6, removing 1 and 2 rank values, but including 8
...
public:
    ...
    static parameters_t get_initial_parameters()
    {
        ...
        PassedPawnPushEnemyBonus.add(result);
        ...
        assert(PassedPawnPushEnemyBonus.bucketTunableSize == 6);
    ...
    static void print_csharp_parameters(const parameters_t &parameters, const std::array<std::array<tune_t, 12>, PSQTBucketCount> &mobilityPieceValues, bool isFinal = false)
    {
        ...
        name = NAME(PassedPawnPushEnemyBonus);
        PassedPawnPushEnemyBonus.to_csharp(parameters, ss, name);
    ...
    static void print_cpp_parameters(const parameters_t &parameters, const std::array<std::array<tune_t, 12>, PSQTBucketCount> &mobilityPieceValues, bool isFinal = false)
    {
        ...
        name = NAME(PassedPawnPushEnemyBonus);
        PassedPawnPushEnemyBonus.to_cpp(parameters, ss, name);
...
int Threats(const chess::Board &board, const chess::Color &color, coefficients_t &coefficients, const std::array<u64, 12> &attacks, const std::array<u64, 2> &attacksBySide, int bucket, int oppositeSideBucket)
{
    ...
    while (safePushes != 0)
    {
        ...
        if ((passedPawnMask & theirPawns) == 0)
        {
            ...
            packedBonus += PassedPawnPushEnemyBonus.packed(oppositeSideBucket, rank);
            IncrementCoefficients(coefficients, PassedPawnPushEnemyBonus.index(oppositeSideBucket, rank - PassedPawnPushEnemyBonus.start), color);
        }
    }
